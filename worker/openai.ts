...
const RESPONSES_TOOL_SYSTEM_DIRECTIVE = [
  ...
  "The client owns local tool execution. When local inspection, shell commands, or file changes are needed, request a tool call and wait for its returned output.",
  "When the input includes function_call_output or custom_tool_call_output records, treat them as completed local tool results for your previous requests and continue from those results.",
...
export function prepareChatRequest(body: unknown, cursorModel: { id: string } | undefined, options: { forceAgentMode?: boolean } = {}): PreparedRequest {
  ...
  const workspaceMutationIntent = workspaceMutationRequired && !wantsWorkspaceInspection(latestUserText);
  ...
  appendWorkspaceMutationRequirement(transcript, workspaceMutationRequired, workspaceMutationDone, tools, latestUserText);
  ...
  for (const message of messages) {
    ...
    if (role === "tool") {
      ...
      transcript.push(`${role.toUpperCase()}: ${workspaceMutationIntent && role === "user" ? addWorkspaceActionToUserText(text) : text || "[empty]"}`);
    }
...
export function prepareResponsesRequest(
  ...
  const tools = record.tool_choice === "none"
    ? []
    : dedupeToolSpecs([
        ...parseChatTools(record.tools),
        ...parseChatTools(responseAdditionalTools(record.input))
      ]);
...
export function responseObject(input: {
  ...
  for (const [index, toolCall] of (input.toolCalls ?? []).entries()) {
    const customCall = codeModeCustomToolCall(toolCall);
    output.push(customCall
      ? {
          id: `ctc_${input.id.slice(5)}_${index}`,
...
export function responseToolCallEvents(input: {
  ...
  if (customCall) {
    const item = {
      id: `ctc_${input.id.slice(5)}_${input.outputIndex}`,
      type: "custom_tool_call",
      status: "in_progress",
...
function codeModeCustomToolCall(toolCall: OpenAiToolCall): { name: string; input: string } | undefined {
  if (normalizeToolName(toolCall.function.name) !== "exec") return undefined;
  const args = parseToolCallArguments(toolCall.function.arguments);
  const code = firstStringArgAllowEmpty(args, "code", "input", "source");
  return code === undefined ? undefined : { name: toolCall.function.name, input: code };
}
...
function parseChatTools(value: unknown): OpenAiToolSpec[] {
  ...
  return value.flatMap((tool, index) => {
    ...
    if (type === "namespace") {
      const namespace = typeof record.name === "string" ? normalizeToolName(record.name) : "";
      return namespace === "functions" ? parseChatTools(record.tools) : [];
    }
    ...
    const rawDescription = typeof fn.description === "string"
      ? fn.description
    ...
    const description = type === "custom"
      ? [
          rawDescription,
          "Cursor SDK invocation: call this tool with a JSON object containing the raw custom-tool source in the `code` field."
        ].filter(Boolean).join("\n\n")
      : rawDescription;
      ...
      ?? (type === "custom"
        ? {
            ...
            properties: {
              code: {
                type: "string",
                description: "Raw source text for this custom tool."
              }
            },
            required: ["code"],
...
function responseAdditionalTools(input: unknown): unknown[] | undefined {
...
function dedupeToolSpecs(tools: OpenAiToolSpec[]): OpenAiToolSpec[] {
  const byName = new Map<string, OpenAiToolSpec>();
...
function appendResponsesToolInventory(transcript: string[], tools: OpenAiToolSpec[], toolChoice: unknown, context?: ToolCallContext) {
  ...
  if (hasCodeModeExecTool(tools)) {
    transcript.push(
      "CODEX CODE MODE:",
      "The SDK custom tool `exec` is directly callable. Call exec itself, not SDK mcp.",
      "Pass one JSON argument named `code` containing raw JavaScript.",
      "For project inspection, use code like: const r = await tools.shell_command({\"command\":\"Get-ChildItem -Name\"}); text(r);",
      "The adapter will return this as a Responses custom_tool_call so Codex can execute it locally."
    );
  }
...
function sdkRoutingRecords(tools: OpenAiToolSpec[], context?: ToolCallContext): Record<string, unknown>[] {
  ...
  for (const tool of tools) {
    if (normalizeToolName(tool.name) === "exec") {
      routes.push({
        sdk: "exec",
        client: tool.name,
        clientArgs: {
          code: "raw JavaScript source; call tools.shell_command/read/etc. and pass the result to text(...)"
        }
      });
      continue;
    }
...
function appendResponsesWorkspaceMutationRequirement(
  ...
  if (!required) return;
  if (wantsWorkspaceInspection(latestUserText)) {
    transcript.push(
      "",
      "LOCAL WORKSPACE INSPECTION REQUIRED:",
      "The latest user request asks about the current project or codebase. Do not answer from general knowledge and do not claim local tools are unavailable.",
      done
        ? "At least one local inspection tool result is already present. Continue from those results, call more read-only tools if needed, then answer with evidence from the workspace."
        : hasCodeModeExecTool(tools)
          ? "Emit exactly one exec custom-tool call next and no prose. Its code must call tools.shell_command with a read-only project inspection command and pass the returned value to text(...)."
        : hasWorkspaceInspectionCapability(tools)
          ? "Emit exactly one SDK read/search/shell tool call next and no prose. The call must be forwarded to an available client tool; after function_call_output returns, continue inspecting as needed."
          : "Emit exactly one SDK mcp call for an available client inspection tool next and no prose; after function_call_output returns, continue inspecting as needed."
    );
    return;
  }
...
function appendWorkspaceMutationRequirement(
  ...
  if (wantsWorkspaceInspection(latestUserText)) {
    transcript.push(
      "",
      "WORKSPACE INSPECTION REQUIRED:",
      "The user is asking about the current project or codebase. Inspect it with the client's read/search/shell tools instead of answering from general knowledge.",
      done
        ? "A local inspection result is already present. Use it, inspect further if needed, then answer with concrete workspace findings."
        : hasWorkspaceInspectionCapability(tools)
          ? "No local inspection has run yet. Your next assistant response must be a read/search/shell tool call, not prose."
          : "No local inspection has run yet. Your next assistant response must use an available client inspection tool, not prose."
    );
    return;
  }
  transcript.push(
...
function appendSdkWorkspaceMutationRequirement(
  ...
  if (wantsWorkspaceInspection(latestUserText)) {
    transcript.push(
      "",
      "SDK WORKSPACE INSPECTION REQUIRED:",
      "The user is asking about the current project or codebase. Do not answer from general knowledge and do not say local tools are unavailable.",
      done
        ? "A client inspection tool result is already present. Continue from it, inspect further if needed, then answer with concrete workspace findings."
        : hasWorkspaceInspectionCapability(tools)
          ? "Your next response must be exactly one SDK read/search/shell tool call that maps to a client tool, with no prose."
          : "Your next response must be exactly one SDK mcp call for an available client inspection tool, with no prose."
    );
    return;
  }
  const requestedTool = explicitlyRequestedToolName(latestUserText, tools);
...
function requestedToolHint(toolName: string): string {
  if (normalizeToolName(toolName) === "exec") {
    return "Use the SDK custom tool exec directly with a `code` argument containing raw JavaScript. Do not route exec through SDK mcp.";
  }
...
function hasWorkspaceMutationCapability(tools: OpenAiToolSpec[]): boolean {
  return hasCodeModeExecTool(tools) || hasAnyCompatibleTool(["write", "shell"], tools);
}

function hasWorkspaceInspectionCapability(tools: OpenAiToolSpec[]): boolean {
  return hasCodeModeExecTool(tools) || hasAnyCompatibleTool(["read", "grep", "glob", "ls", "semsearch", "shell"], tools);
}

function hasCodeModeExecTool(tools: OpenAiToolSpec[]): boolean {
  return tools.some((tool) => normalizeToolName(tool.name) === "exec");
}
...
function isKnownMappedToolName(name: string): boolean {
  return new Set([
    "bash", "shell", "shellcommand", "terminal", "runterminalcmd", "runterminalcommand", "runshellcommand",
...
function responseInputToTextAndImages(input: unknown, tools: OpenAiToolSpec[] = []): { text: string; images: CursorImage[] } {
  ...
  for (const item of input) {
    ...
    if (record.type === "message" || typeof record.role === "string") {
    ...
    } else if (record.type === "function_call" || record.type === "custom_tool_call") {
      ...
      const args = record.type === "custom_tool_call"
        ? { code: typeof record.input === "string" ? record.input : responseInputText(record.input) }
        : parseToolCallArguments(record.arguments);
...
function shouldRequireLocalTool(text: string, tools: OpenAiToolSpec[]): boolean {
  ...
  if (wantsWorkspaceInspection(text) && hasWorkspaceInspectionCapability(tools)) return true;
  const lower = text.toLowerCase();
  const hasPathSignal =
    lower.includes("~/") ||
    lower.includes("/") ||
...
function hasRequiredLocalToolCall(messages: unknown[], tools: OpenAiToolSpec[], latestUserText: string): boolean {
  ...
  if (wantsWorkspaceInspection(latestUserText)) return hasWorkspaceInspectionToolCallAfterLatestUser(messages);
...
function hasRequiredResponseLocalToolCall(input: unknown, tools: OpenAiToolSpec[], latestUserText: string): boolean {
  ...
  if (wantsWorkspaceInspection(latestUserText)) return hasResponseWorkspaceInspectionToolCall(input);
  return hasResponseWorkspaceMutationToolCall(input, tools);
}

function wantsWorkspaceInspection(text: string): boolean {
  const lower = text.toLowerCase();
  const englishAction = /\b(understand|inspect|explore|review|analy[sz]e|examine|read|summari[sz]e|explain|familiarize|map)\b/.test(lower);
  const englishTarget = /\b(codebase|repo|repository|project|source|code|implementation|architecture|structure|directory|files?)\b/.test(lower);
  const chineseAction = /(了解|熟悉|查看|检查|阅读|分析|梳理|总结|解释|探索|审查|看看|读一下)/.test(text);
  const chineseTarget = /(项目|代码库|代码|仓库|目录|文件|实现|架构|结构)/.test(text);
  return (englishAction && englishTarget) || (chineseAction && chineseTarget);
}

function latestUserTextFromMessages(messages: unknown[]): string {
  for (const message of [...messages].reverse()) {
    if (!isRecord(message) || message.role !== "user") continue;
    return userIntentText(contentToPlainText(message.content));
  }
  return "";
}
...
function latestUserTextFromResponseInput(input: unknown): string {
  ...
  for (const item of [...input].reverse()) {
    ...
    if (item.type === "message" || typeof item.role === "string") {
      const role = typeof item.role === "string" ? item.role : "user";
      if (role === "user") return userIntentText(contentToPlainText(item.content));
    }
  }
  return "";
}

function userIntentText(text: string): string {
  return text
...
function hasWorkspaceInspectionToolCallAfterLatestUser(messages: unknown[]): boolean {
  let sawLatestUser = false;
  ...
  for (const message of messages) {
    ...
    for (const toolCall of message.tool_calls) {
      if (!isRecord(toolCall)) continue;
      const fn = isRecord(toolCall.function) ? toolCall.function : undefined;
      if (typeof fn?.name === "string" && isWorkspaceInspectionToolName(fn.name)) {
...
function hasResponseWorkspaceMutationToolCall(input: unknown, tools: OpenAiToolSpec[] = []): boolean {
  ...
  for (const item of input) {
    ...
    if (!sawLatestUser || !["function_call", "custom_tool_call"].includes(String(item.type)) || typeof item.name !== "string") continue;
    const args = item.type === "custom_tool_call" ? { code: item.input } : item.arguments;
    if (isWorkspaceMutationToolCall(item.name, args, tools)) mutationAfterLatestUser = true;
  }
  return mutationAfterLatestUser;
}

function hasResponseWorkspaceInspectionToolCall(input: unknown): boolean {
  if (!Array.isArray(input)) return false;
  let sawLatestUser = false;
  let foundAfterLatestUser = false;
  for (const item of input) {
    ...
    if (
      ...
      && isWorkspaceInspectionToolName(item.name)
    ) {
      foundAfterLatestUser = true;
    }
  }
...
function isWorkspaceInspectionToolName(name: string): boolean {
  return normalizeToolName(name) === "exec"
    || ["read", "grep", "glob", "ls", "semsearch", "shell"].includes(canonicalToolName(name));
}
...
function hasSpecificResponseToolCallAfterLatestUser(input: unknown, requestedTool: string, tools: OpenAiToolSpec[] = []): boolean {
  ...
  for (const item of input) {
    ...
    const args = item.type === "custom_tool_call"
      ? { code: typeof item.input === "string" ? item.input : responseInputText(item.input) }
      : parseToolCallArguments(item.arguments);
    if (toolCallMatchesClientTool(item.name, args, requestedTool, tools)) {
      foundAfterLatestUser = true;
    }
  }
  return foundAfterLatestUser;
...
function canonicalToolName(value: string): string {
  ...
  if (["bash", "shellcommand", "runshellcommand", "runterminalcommand", "runterminalcmd", "terminal", "execute", "executecommand", "runcommand", "run"].includes(normalized)) {
    return "shell";
  }
