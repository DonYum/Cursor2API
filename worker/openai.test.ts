...
describe("OpenAI compatibility adapter", () => {
  ...
  it("requires a forwarded Codex shell tool when inspecting the current project", () => {
    const prepared = prepareResponsesRequest(
      {
        model: "composer-2.5",
        input: [
          { role: "user", content: [{ type: "input_text", text: "了解当前项目代码" }] }
        ],
        tools: [
          {
            type: "function",
    ...
    expect(prepared.requiresLocalTool).toBe(true);
    expect(prepared.prompt.text).toContain("LOCAL WORKSPACE INSPECTION REQUIRED:");
    expect(prepared.prompt.text).toContain("Client tool targets: shell_command");
    expect(prepared.prompt.text).toContain("exactly one SDK read/search/shell tool call");
    expect(prepared.prompt.text).not.toContain("WORKSPACE MUTATION REQUIRED:");
  });

  it("supports Codex Code Mode tools from Responses additional_tools items", () => {
    const input = [
      {
            ...
            tools: [
              {
                type: "custom",
                name: "exec",
                description: "Run JavaScript with nested tools including tools.shell_command.",
      ...
      { role: "user", content: [{ type: "input_text", text: "分析当前项目" }] }
    ];
    const prepared = prepareResponsesRequest({ model: "gpt-5.6-sol", input }, { id: "gpt-5.6-sol" });

    expect(prepared.tools.map((tool) => tool.name)).toEqual(["exec", "wait"]);
    expect(prepared.prompt.mode).toBe("agent");
    expect(prepared.requiresLocalTool).toBe(true);
    expect(prepared.prompt.text).toContain("CODEX CODE MODE:");
    expect(prepared.prompt.text).toContain("Emit exactly one exec custom-tool call next");
    expect(prepared.prompt.text).not.toContain("DEVELOPER: [empty]");

    const [toolCall] = toOpenAiToolCalls({
      responseId: "resp_code_mode",
      tools: prepared.tools,
      toolCalls: [{
        name: "exec",
        arguments: {
          code: 'const r = await tools.shell_command({"command":"Get-ChildItem -Name"}); text(r);'
        }
      }]
    });
    ...
    expect(response.output).toEqual([
      expect.objectContaining({
        ...
        name: "exec",
        call_id: toolCall.id,
        input: 'const r = await tools.shell_command({"command":"Get-ChildItem -Name"}); text(r);'
      })
    ]);
  });

  it("continues after a Codex Code Mode custom tool result", () => {
    const prepared = prepareResponsesRequest(
      {
        model: "gpt-5.6-sol",
        input: [
          {
            type: "additional_tools",
            role: "developer",
            tools: [{ type: "custom", name: "exec", description: "Run JavaScript with nested tools." }]
          },
          { role: "user", content: [{ type: "input_text", text: "分析当前项目" }] },
          {
            type: "custom_tool_call",
          ...
          },
          {
            ...
            call_id: "call_exec",
            output: [{ type: "input_text", text: "build.gradle.kts\napp\nREADME.md" }]
          }
        ]
      },
      { id: "gpt-5.6-sol" }
    ...
    expect(prepared.requiresLocalTool).toBe(false);
    expect(prepared.prompt.text).toContain("At least one local inspection tool result is already present");
    expect(prepared.prompt.text).toContain("build.gradle.kts");
  });

  it("requires a forwarded Claude Code Bash tool when inspecting the current project", () => {
    const prepared = prepareChatRequest(
      {
        model: "claude-fable-5",
        messages: [{ role: "user", content: "了解当前项目代码" }],
        tools: [{
          type: "function",
          function: {
            name: "Bash",
    ...
    expect(prepared.requiresLocalTool).toBe(true);
    expect(prepared.prompt.text).toContain("WORKSPACE INSPECTION REQUIRED:");
    expect(prepared.prompt.text).toContain("Your next assistant response must be a read/search/shell tool call");
    expect(prepared.prompt.text).not.toContain("WORKSPACE MUTATION REQUIRED:");
    expect(prepared.prompt.text).not.toContain("Workspace action required:");
  });

  it("does not infer a tool requirement from Claude Code system-reminder context", () => {
    const prepared = prepareChatRequest(
      {
        model: "claude-opus-5",
        messages: [{
          ...
          content: [
            "<system-reminder>",
            "Project guidance may mention create files, run shell commands, and paths such as /tmp/project.",
            "IMPORTANT: this context may or may not be relevant to your tasks.",
            "</system-reminder>",
            "",
            "你是什么模型？只返回模型名称和上下文档位。"
          ].join("\n")
        }],
        tools: [{
          type: "function",
      ...
      { id: "claude-opus-5[context=1m]" }
    );

    expect(prepared.requiresLocalTool).toBe(false);
    expect(prepared.prompt.text).not.toContain("attempt 1 of 3");
    expect(prepared.prompt.text).not.toContain("WORKSPACE INSPECTION REQUIRED:");
  });

  it("continues after the forwarded Codex inspection tool returns", () => {
    const prepared = prepareResponsesRequest(
      {
        model: "composer-2.5",
        input: [
          { role: "user", content: [{ type: "input_text", text: "了解当前项目代码" }] },
          {
            ...
            call_id: "call_shell",
            name: "shell_command",
            arguments: JSON.stringify({ command: "rg --files", workdir: "C:\\project" })
          },
          { type: "function_call_output", call_id: "call_shell", output: "package.json\nsrc/index.ts" }
        ],
        tools: [
          {
            type: "function",
